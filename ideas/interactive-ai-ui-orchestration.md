...
**Как читать документ:** продукт и TL;DR → глоссарий → идея и схема цикла → пресеты → архитектура и **UI Tool** → голос
и UX → модели → примеры потоков → перспективы и сторонние библиотеки.
...
Детали проводки цикла и контрактов — в разделе **«🏗️ Архитектура»**.
...
Ниже — **слои системы**: доступ к моделям через общий модуль, **агент** с **AgentLoop**, контракт **UI Tool** и
остальные **tools**. Раздел задаёт каркас; детали полей формы — в **«UI Tool: элементы интерфейса»**.

### 📦 Пакет `ai` (модуль в workspace)

В монорепозитории предусмотрен **отдельный пакет** (например **`ai`** в workspace) — **абстракция над языковыми моделями
и провайдерами**. Через него приложение получает **единый интерфейс** к:

- **текстовым** моделям (чат, инструкции, оркестрация);
- **генерации изображений** (и при необходимости смежным вызовам на бэкенде);
- **structured output** (JSON по схеме, в т.ч. для **UI Plan**);
- **вызовам инструментов** со стороны модели (**tool** / **function calling**), согласованным с **AgentLoop**;
- валидации и при необходимости **исправлению** ответов под схему.

Приложение **не привязано** к одному вендору: конкретные endpoint, ключи и идентификаторы моделей задаются **конфигом**,
а не кодом фич. Ориентиры по **локальным моделям** и **облачным провайдерам** — в разделе **«🤖 Модели»**.

### 🔁 Агент и AgentLoop

**Агент** — слой, который:

| Что делает        | Содержание                                                                                                                                |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Системные промпты | Роль, домен, ограничения, формат ответов, **как вызывать tools** (включая **UI Tool**), критерии успешного исхода.                        |
| **AgentLoop**     | Цикл: модель возвращает рассуждение и/или **вызовы tools** → среда исполняет tools → результаты подмешиваются в контекст → следующий ход. |

**Модель не получает жёсткого сценария** «сначала A, потом B». Она видит список tools и правила в промптах и **сама
решает**, нужен ли **UI Tool**, сколько раз вызывать генерацию, можно ли обойтись без формы. Агент обеспечивает
**контракт исполнения**: аргументы tool, рендер UI по **UI Plan**, возврат **`answers`** в цикл.

### 📱 UI Tool в цикле

**UI Tool** — показ экрана **выбора параметров**: клиент строит форму по декларации и кнопку **«Продолжить»** /
**«Сгенерировать»**. После подтверждения в контекст возвращаются **`answers`**; **следующий ход снова определяет
модель** (те же или другие tools, повторный UI при необходимости).

Детали **Field kind** и разметки — в **«📋 UI Tool: элементы интерфейса»**.

### 🛠️ Прочие tools

Примеры операций среды (не исчерпывающий список):
...
- доменные действия с явным контрактом аргументов.
...
По **UI Plan** клиент рисует **форму**:

| Часть экрана        | Содержание                                                                                                                                                                                 |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 📌 **Шапка**        | Заголовок и при необходимости вводный текст к форме.                                                                                                                                       |
| 📐 **Строки полей** | В каждой строке **слева** — метка **`label`**, **справа** — компонент по **`kind`**. У поля есть **`id`** (ключ в **`answers`**) и параметры из схемы (**Field kind**, см. **глоссарий**). |
| ✅ **Действие**     | Главная кнопка подтверждения (не отдельный **`kind`** поля) — раздел **«Подтверждение экрана»** в конце справочника.                                                                       |

**Содержимое формы** — набор полей с **`kind`** из справочника ниже. Логика разделов:

- **Ввод текста и чисел** — `text`, `textarea`, `number`
- **Выбор значений** — `toggle`, `tabs_single`, `tabs_multi`
- **Визуал и вложения** — `color`, `file`
- **Данные для диаграмм** — `chart_series`
- **Подтверждение экрана** — отдельно от полей

---

### ✏️ Ввод текста и чисел

#### 📝 `text`

- **🎯 Назначение:** короткая строка — имя, заголовок, одно значение без переносов.
- **🖥️ Интерфейс:** справа от метки — **однострочное поле ввода**.
- **📤 `answers`:** одна строка по **`id`** поля. Голосовой ввод — **«🎙️ Голосовой режим»**.

#### 📄 `textarea`

- **🎯 Назначение:** многострочный текст — описание, черновик, комментарий.
- **🖥️ Интерфейс:** справа от метки — **многострочная область** (высота по продукту, в т.ч. с автоподстройкой).
- **📤 `answers`:** строка с переносами по **`id`**. Голос — в **«🎙️ Голосовой режим»**.

#### 🔢 `number`

- **🎯 Назначение:** число — количество, возраст, порог, коэффициент.
- **🖥️ Интерфейс:** справа от метки — **ввод числа** (на мобильном — цифровая клавиатура); опционально степпер или
  слайдер.
- **📤 `answers`:** число по **`id`**. Границы **`min`** / **`max`** — в **UI Plan**, если нужны.
...
- **🎯 Назначение:** один логический флаг — вкл/выкл, да/нет и т.п.
- **🖥️ Интерфейс:** справа от метки — **переключатель** (два состояния).
- **📤 `answers`:** булево значение по **`id`**.

#### 📑 `tabs_single`

- **🎯 Назначение:** выбрать **ровно один** вариант из набора (тип диаграммы, тон, одна палитра).
- **🖥️ Интерфейс:** справа от метки — **ряд вкладок**; активна одна опция.
- **📤 `answers`:** **`id`** выбранной опции. У опций — **`id`**, **`label`**, опционально **иконка** и **превью**.
  Набор может приходить из **`artifacts`** (см. **Artifact** в глоссарии), например `palette_candidates`.

#### 🏷️ `tabs_multi`

- **🎯 Назначение:** выбрать **несколько** вариантов сразу.
- **🖥️ Интерфейс:** справа от метки — **вкладки или чипы**; несколько вариантов видны как выбранные.
- **📤 `answers`:** массив **`id`** опций по **`id`** поля. Структура опций как у **`tabs_single`**; **`artifacts`** —
  по необходимости.
...
- **🎯 Назначение:** цвет — акцент, фон, обводка.
- **🖥️ Интерфейс:** справа от метки — **палитра / пресеты / пипетка** (по продукту) и индикатор значения.
- **📤 `answers`:** цвет в согласованном формате (например HEX) по **`id`**. Стыковка с **темой** — на клиенте.

#### 📎 `file`

- **🎯 Назначение:** вложение — референс, таблица, файл задачи.
- **🖥️ Интерфейс:** справа от метки — **кнопка выбора** и/или **drag-and-drop**; имя и тип файла (лимиты — по продукту).
- **📤 `answers`:** ссылка на ресурс или id в хранилище по **`id`** поля.
...
- **🎯 Назначение:** несколько **рядов данных** — подписи, значения, колонки для диаграммы.
- **🖥️ Интерфейс:** справа от метки — **таблица или сетка**; добавление строк по продукту.
- **📤 `answers`:** структура рядов (JSON по контракту) по **`id`**. Допустим совместимый **`kind`**, если так
  зафиксировано в схеме.

---

### ✅ Подтверждение экрана

Это **не** значение **`kind`** в поле — отдельный элемент макета.

- **🎯 Назначение:** зафиксировать ввод и отправить **`answers`** в **AgentLoop**.
- **🖥️ Интерфейс:** одна **главная кнопка** на ширину формы (внизу или в шапке); подпись — **«Продолжить»** или
  **«Сгенерировать»**.
- **📤 Дальше:** клиент собирает все поля по **`id`**. Следующий ход (**tools**, повторный **UI Tool**) решает
  **модель**.
...
> **📎 Контракт:** полный **UI Plan** фиксируется **JSON Schema** в репозитории. Поведение клиента при неизвестном
> **Field kind** задаётся явно (например запасной рендер или пропуск поля — по политике продукта).
...
Здесь — **откуда брать модели** в dev и prod: локальный **Ollama** и примеры облачных **провайдеров**. Как к ним
подключаться из приложения и где задаётся конфиг — в **«🏗️ Архитектура»**; в этом разделе — **источники**, теги и
ориентиры по железу.

### ⚙️ Режимы: разработка и продакшен

| Режим    | Источник моделей                                                                         | Назначение                                                                                    |
| -------- | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| **Dev**  | **Ollama** (локально)                                                                    | Быстрые итерации без облачных ключей; те же сценарии вызова, что в проде, на локальных тегах. |
| **Prod** | Провайдеры с HTTP API, совместимые с OpenAI Chat Completions / Images там, где применимо | Модели и ключи из окружения, не из кода фич.                                                  |

**Идея:** в dev — локальный рантайм (**Ollama**), в prod — HTTP API с `baseURL` и ключами из конфигурации; переключение
без смены прикладной логики агента.
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **`qwen2.5:7b`**  | Удобный **дефолт**: инструкции и структурированный вывод, **многоязычность** (в т.ч. русский); 7B на 12 ГБ обычно с запасом. |
| **`llama3.1:8b`** | Альтернатива Meta: предсказуемый общий чат и инструкции.                                                                     |
| **`qwen2.5:3b`**  | Лёгкий вариант: быстрее и меньше VRAM; сложная схема UI может быть слабее.                                                   |

**Генерация изображений:**

| Модель (Ollama)     | Когда уместна                                                                              |
| ------------------- | ------------------------------------------------------------------------------------------ |
| **`flux:schnell`**  | Баланс качества и скорости в **Flux**; удобный **дефолт** для старта в Ollama.             |
| **`z-image-turbo`** | Упор на **скорость** и VRAM; заметна на задачах с **текстом на изображении**.              |
| **`sdxl`**          | Запасной вариант: предсказуемое поведение, если другие модели дают OOM или нестабильность. |
...
| **Подтверждение** | Пользователь подтверждает экран (**«Продолжить»** / **«Сгенерировать»**); в **AgentLoop** передаются **`answers`** по `id` полей **UI Plan**.                                                                                                                                                                                                                              |
| **Дальше**        | **Tool** генерации **растра** по **`answers`** → **tool** **векторизации** в **SVG** (клиент или сервер — по продукту), типично для **контрастного** растра, удобного для контура → **SVG** в ленту или скачивание. Это **не** путь **data**-диаграммы — см. колонку **Дальше** в **«📊 Диаграмма / график»**. Цветной знак с плавными переходами — см. **«SVG логотип»**. |
