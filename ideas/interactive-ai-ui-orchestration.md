...
### 🔄 Иллюстрация потока (не протокол)

Ниже — **схема для человека**, а не инструкция исполнителю. Пропуски, повторы и другой порядок допустимы.

```text
Системный контекст + (пресет | свободный текст пользователя)
        ↓
   [решение модели]
        ↓
  при необходимости — UI Tool → пользователь заполняет экран → answers в AgentLoop
        ↓
   [решение модели]
        ↓
  вызовы tools / текстовый ответ пользователю → при необходимости новый круг
```

На практике **модель** сама выбирает, сколько раз войти в **UI Tool**, нужен ли он вообще и какие **tools** вызвать до
или после сбора параметров.
