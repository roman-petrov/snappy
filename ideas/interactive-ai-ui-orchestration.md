...
| Шаг | Содержание |
| --- | ---------- |
| **Намерение** | Например: _«Столбчатый график выручки по кварталам»_, круговая или линейная диаграмма по тем же данным. |
| **Контекст** | Пресет «диаграмма» или **Free mode** с тем же текстом намерения. |
| **UI Plan** | Заголовок (**`text`**); подписи оси X и значения (**`textarea`** или **`chart_series`**); тип (**`tabs_single`**: столбцы, круг, линия…); при необходимости единицы (**`text`**); палитра из **`artifacts`** (**`palette_candidates`**) — **`tabs_single`** с превью. При необходимости — и другие поля визуала по продукту (стиль, фотореализм, назначение **иконка** / **иллюстрация** и т.д.). Всё может уместиться **в одном** UI Plan. |
| **Подтверждение** | Кнопка «Продолжить» / «Сгенерировать» → в цикл уходят **`answers`**. |
| **Дальше** | Tool рендера диаграммы: например **`chart_spec`** (Vega-Lite и аналоги), готовый **SVG** или иной тип результата по продукту (3D-bar и т.п.); **клиент** строит **читаемую data-визуализацию** по spec/данным, а не «угадывает» в чате. В метаданных tool — явные `name` / `args` и тип результата, чтобы клиент маршрутизировал вызов на **движок диаграмм** и не дублировал логику формы. **Не путать** с генерацией **растровой картинки**, похожей на график, если нужна именно диаграмма по данным. |
| **Иначе** | Модель может уточнить в диалоге, показать **меньше** полей или **другой** порядок tools. Растровая генерация изображений и путь **иконка → вектор (SVG)** — в примере **«Изображение → SVG»**; там же типичный **растр** по полям **UI Tool** и отличие от покадровой «фото»-генерации для иконок. |
...
| **Дальше**        | **Tool** генерации **растра** по согласованным полям → при цели «чистый вектор» **tool** **векторизации** растра в **SVG** (на клиенте или на сервере — по продукту) → **SVG** в ленту или скачивание. От **data**-диаграммы (spec, движок) этот путь отличается — см. **Дальше** в примере **«Графики / диаграммы»**. |
...
- **Potrace** — [оригинал на SourceForge](http://potrace.sourceforge.net/), [potrace-wasm на GitHub](https://github.com/IguteChung/potrace-wasm) — классическая трассировка монохромного растра в векторные контуры;
  часто используется как эталон поведения; есть порты и обёртки под разные среды.
