...
const sampleReveal = async (revealed: () => number, ms: number) => {
  ...
  while (performance.now() - start < ms) {
    await nextFrame();
    samples.push({ chars: revealed(), seconds: (performance.now() - start) / 1000 });
  }
...
describe(`push`, () => {
  it(`resolves true for empty content without host`, async () => {
    ...
    await expect(writer.push(``)).resolves.toBe(true);
  });
  ...
  it(`resolves true for void-only html`, async () => {
    ...
    await expect(writer.push(`<hr>`)).resolves.toBe(true);
  });
  ...
  it(`types text and resolves true with host`, async () => {
    ...
    await expect(writer.push(`<p>hello</p>`)).resolves.toBe(true);
  ...
  it(`resolves repeated identical html`, async () => {
    ...
    await expect(writer.push(`<p>same</p>`)).resolves.toBe(true);
    await expect(writer.push(`<p>same</p>`)).resolves.toBe(true);
  ...
  it(`hides empty trailing blocks under the caret`, async () => {
    ...
    await expect(writer.push(`<p>done</p><p></p>`)).resolves.toBe(true);
  ...
  it(`drops the bottom margin of the last visible block while trailing blocks are hidden`, async () => {
    ...
    await expect(writer.push(`<p>done</p><p>more</p>`)).resolves.toBe(true);
  ...
  it(`retypes from the divergence point when content changes`, async () => {
    ...
    await expect(writer.push(`<p>abcdef</p>`)).resolves.toBe(true);
    await expect(writer.push(`<p>abcXYZ</p>`)).resolves.toBe(true);
  ...
  it(`reveals buffered content at linear speed, keeping the caret visible`, async () => {
    ...
    while (performance.now() - start < 350) {
      ...
      expect(revealed()).toBeLessThanOrEqual(elapsed * fastCharsPerSecond + 3);
  ...
  it(`reveals a growing table cell smoothly`, async () => {
    ...
    const writer = fastWriter(hostElement(cell ?? document.body));
    ...
    const samples = await sampleReveal(chars(document, `td`), 450);
    stop();
    writer.destroy();
  ...
  it(`reveals a growing list item smoothly`, async () => {
    ...
    const writer = fastWriter(hostElement(item ?? document.body));
    ...
    const samples = await sampleReveal(chars(document, `li`), 450);
    stop();
    writer.destroy();
  ...
  it(`measures each grapheme once across a growing stream`, async () => {
    ...
    for (let sent = 6; sent < full.length + 6; sent += 6) {
      await writer.push(`<p>${full.slice(0, sent)}</p>`);
    }
  ...
  it(`continues without a burst after waiting for data`, async () => {
    ...
    await expect(writer.push(`<p>${`a`.repeat(10)}</p>`)).resolves.toBe(true);
...
describe(`setWaiting`, () => {
  it(`shows the caret while waiting without content`, () => {
    const host = hostElement();
    const writer = TypeWriter();
    writer.attach(host);
    writer.setWaiting(true);
...
describe(`attach`, () => {
  it(`shows a waiting caret instead of stale content on a new host`, async () => {
    ...
    await expect(writer.push(`<p>old segment</p>`)).resolves.toBe(true);
