...
export const Pill = ({ hint, name }: PillProps) => (
  <div className={_.cn(styles.pill, $.surface(`surface`), $.elevation(`e1`), $.radius(`md`))}>
    <Text cn={_.cn(styles.pillLeft, $.surface(`primary`), $.elevation(`e2`))} text={name} typography="captionBold" />
    <Text cn={_.cn(styles.pillRight, $.surface(`primary`), $.elevation(`e2`))} text={hint} typography="large" />
  </div>
);
