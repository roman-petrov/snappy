...
export const TranscribeAudioTool: SnappyToolFactory = ({ ai, config, files, isStopped }) =>
  AgentTool({
    description: [
      [
        `when`,
      ...
      [`input`, `Pass the field id from an ask file_input answer. Request the audio file via ask first.`],
    ...
    inputSchema: z.object({ file: z.string().min(1).describe(`Field id from ask file_input answer.`) }),
  });
