...
export type OverlayPane = { active: boolean; pattern: string; stacked: boolean; state: RouterPageState };
...
const stage = ({ current, layerOf, parent, path, pattern, preserved: previous, stateAt }: StageInput) => {
  ...
  if (layer !== `cover`) {
    return { idle, open: false, panes: [], preserved };
  }
  ...
  const panes = items.map((pane, index) => ({ ...pane, active: index === last, stacked: stacked && index === last }));

  return { idle, open, panes, preserved };
};

export const RouteOverlay = { stage };
