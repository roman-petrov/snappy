...
describe(`apply`, () => {
  describe(`emphasis`, () => {
    ...
    it(`does not close emphasis across escaped markers`, () => {
      expect(apply(String.raw`Escape check: \*not`)).toBe(String.raw`Escape check: \*not`);
      expect(apply(String.raw`Escape check: \*not bold\*`)).toBe(String.raw`Escape check: \*not bold\*`);
      expect(apply(String.raw`Escape check: \_not`)).toBe(String.raw`Escape check: \_not`);
      expect(apply(String.raw`Escape check: \`not`)).toBe(String.raw`Escape check: \`not`);
    });
  });
});
