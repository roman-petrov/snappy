...
describe(`useAiStreamerState`, () => {
  ...
  it(`advances playIndex across html and code segments while animated`, async () => {
    await withTiming(async ({ until }) => {
      ...
      rerender({ networkDone: false, text: markdown });
      await until(() => result.current.playIndex === 0 && result.current.doc.length > 0);
  ...
  it(`keeps completed top pieces in the document behind the play head`, async () => {
    await withTiming(async ({ until }) => {
      ...
      await until(() => result.current.playIndex >= 1 && result.current.doc.length >= 2);

      expect(result.current.doc.length).toBeGreaterThanOrEqual(2);
      expect(result.current.playIndex).toBeGreaterThanOrEqual(1);
    });
  });

  it(`keeps highlighted code html when the play head leaves a code segment`, async () => {
    await withTiming(async ({ until }) => {
      const markdown = `intro\n\n\`\`\`js\nconst answer = 42;\n\`\`\`\n\noutro`;
      const highlighted = `<pre><code>const answer = 42;</code></pre>`;
      const { rerender, result } = renderHook(useHarness, { initialProps: { networkDone: false, text: `` } });
      attachHost(result.current.tailHost);
      ...
      act(() => {
        result.current.pushTailHtml(highlighted);
      });

      await until(() => result.current.playIndex >= 2);

      expect(result.current.codeHtmlByIndex.get(1)).toBe(highlighted);
      expect(result.current.doc.length).toBeGreaterThanOrEqual(2);
    });
  });

  it(`keeps highlighted code html after the stream ends`, async () => {
    await withTiming(async ({ until }) => {
      const markdown = `\`\`\`js\nconst answer = 42;\n\`\`\``;
      const highlighted = `<pre><code>const answer = 42;</code></pre>`;
      const { rerender, result } = renderHook(useHarness, { initialProps: { networkDone: false, text: `` } });
      ...
      expect(result.current.streaming).toBe(false);
      expect(result.current.codeHtmlByIndex.get(0)).toBe(highlighted);
    });
  });
  ...
  it(`resets seal and play state when streaming text clears`, async () => {
    await withTiming(async ({ until }) => {
      ...
      expect(result.current.playIndex).toBe(0);
      expect(result.current.doc).toHaveLength(0);
  ...
  it(`renders static completed text without a waiting host`, async () => {
    await withTiming(async ({ advance }) => {
      ...
      expect(result.current.waitingHost).toBe(false);
      expect(result.current.tailHost).toBeUndefined();
      expect(result.current.doc.length).toBeGreaterThan(0);
    });
  });
  ...
  it(`keeps stream playIndex at the end when trailing text arrives after a code block`, async () => {
    await withTiming(async ({ until }) => {
      ...
      expect(result.current.doc.length).toBeGreaterThan(1);
