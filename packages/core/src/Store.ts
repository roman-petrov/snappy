...
/**
 * Implementation for useSyncExternalStore (https://react.dev/reference/react/useSyncExternalStore)
...
export type StoreApi<TValue> = ReadonlyStoreApi<TValue> & { set: (value: TValue, silent?: boolean) => TValue };
...
export const Store = <TValue>(initialValue: TValue): Store<TValue> => {
  ...
  const set = (value: TValue, silent = false) => {
