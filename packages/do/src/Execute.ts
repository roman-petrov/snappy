...
const run = async (
  ...
  const result = await Run.run(root, command, {
    stdio: isDev ? `inherit` : stdio,
    timeoutMs: isDev ? devTimeoutMs : timeoutMs,
  });

  const message =
    stdio === `inherit` ? `Exited with code ${result.exitCode}.` : Run.formatResult(script, result);
