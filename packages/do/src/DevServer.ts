...
export const DevServer = async () => {
  ...
  type HtmlConfig = {
    body: (input: Cookie & { template: string }) => Promise<string> | string;
    dir: string;
    documentUrl?: string;
    path: RegExp | string;
  ...
  html({
    body: async ({ locale, template, theme }) =>
      SiteSsr.build(locale, theme, template, (await vite.ssrLoadModule(pathToFileURL(siteEntryPath).href)) as SsrEntry),
    dir: siteDir,
    path: `/`,
  });
