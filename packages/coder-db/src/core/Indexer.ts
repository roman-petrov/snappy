...
import type { AiEmbedderModel } from "@snappy/ai";
...
export const Indexer =
  ({ dbDir, embedder, ignore, projectRoot, store }: IndexerConfig) =>
  async ({ onProgress }: SyncOptions = {}) => {
    ...
    if (loadedManifest.embeddingModelName !== embedder.name) {
      await store.clean();
      manifest = {};
      await saveManifest(dbDir, { embeddingModelName: embedder.name, files: manifest });
    }
    ...
    const onFileCompleted = async (currentPath: string) => {
      ...
      if (writesSinceManifestSave >= Constants.indexing.writesUntilManifestFlush) {
        ...
        const nextManifest: IndexManifestState = { embeddingModelName: embedder.name, files: manifest };
        ...
        const producer = async () => {
          ...
          const worker = async () => {
            while (cursor < queue.length) {
              ...
              const { vectors } = await embedder.embed({ input: batch.map(item => item.chunk.text) });
