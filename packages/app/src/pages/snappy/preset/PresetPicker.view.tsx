...
export const PresetPickerView = ({ card, invalid, missingStatic, pageTitle }: PresetPickerViewProps) =>
  invalid || card === undefined ? (
    ...
    <Redirect to={Routes.snappy.preset.chat({ presetId: card.id })} />
  ) : (
    <Page back title={pageTitle} trailing={<BalanceTap />}>
      <div className={styles.root}>
        ...
        <RichCard
          ...
          icon={Sparkles}
          link={Routes.snappy.preset.chat({ presetId: card.id })}
        ...
        <RichCard
          ...
          icon={ClipboardList}
          link={Routes.snappy.preset.static({ presetId: card.id })}
