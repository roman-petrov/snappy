...
export const useSettingsModelsImageState = () => {
  ...
  useAsyncEffect(async () => {
    ...
    const ids =
      modelsResponse.status === `ok`
        ? modelsResponse.settings.models.filter(model => model.type === `image`).map(model => model.name)
        : [];
    setOptions(ids.map(id => ({ icon: `🎨`, label: id, value: id })));
    if (settingsResponse.status === `ok`) {
      const v = settingsResponse.llmImageModel;
  ...
  const onSelect = async (next: string) => {
    const settingsSetResponse = await api.userLlmSettingsSet({ llmImageModel: next });
    if (settingsSetResponse.status === `ok`) {
      setValue(settingsSetResponse.llmImageModel);
    }
  };
