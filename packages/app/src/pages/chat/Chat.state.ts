...
export const useChatState = () => {
  ...
  const [maxPromptImageLength, setMaxPromptImageLength] = useState(0);
  ...
  useAsyncEffect(async () => {
    ...
    if (llmSettingsResponse.status === `ok`) {
      ...
      setMaxPromptImageLength(llmSettingsResponse.maxPromptImageLength);
  ...
  const agent = useMemo(
    ...
    [agentId, hostTools, llm, maxPromptImageLength, maxSpeechFileMegaBytes, phase],
  );
